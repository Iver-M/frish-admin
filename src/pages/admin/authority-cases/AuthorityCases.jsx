...
export default function AuthorityCases() {
  ...
  return <main className="page consumer-intake"><div className="intake-disclosure"><strong>Emulator prototype — not connected to production Firebase.</strong><span>Consumer authority cases are separate from legacy Inspector reports.</span></div>
    ...
    {loading ? <p className="intake-state">Loading emulator authority cases…</p> : caseId ? <CaseDetail record={record} /> : <CaseList records={records} />}
  </main>
}
...
function CaseDetail({ record }) { if (!record) return <p className="intake-state">Authority case not found or not permitted.</p>; return <section className="intake-panel intake-detail"><header><div><p className="workspace-kicker">CONSUMER AUTHORITY CASE</p><h2>{record.title}</h2></div><span className="prototype-badge">{record.status}</span></header><dl><Fact label="Case ID" value={record.caseId} /><Fact label="Source concern" value={record.sourceConcernReportId} /><Fact label="Market" value={record.assignedMarket} /><Fact label="Vendor or stall" value={record.vendorOrStall} /><Fact label="Reason" value={pretty(record.reason)} /><Fact label="Analysis trust" value="Unavailable" /><Fact label="Description" value={record.description || 'None provided'} /></dl><p className="intake-privacy">This general authority view contains no reporter email, anonymous Consumer UID, image data, Storage path, local URI, public URL, or temporary model result.</p><div className="intake-actions"><Link className="btn btn-outline" to="/authority-cases">Back to authority cases</Link><Link className="btn btn-outline" to="/reports">Open legacy Inspector reports</Link></div></section> }
